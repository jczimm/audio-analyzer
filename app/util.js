...
util.copyFile = function copyFile(sourcePath, destDir) {
    return new Promise(function (resolve, reject) {
        ...
        var destPath = _path2['default'].resolve(destDir, _path2['default'].basename(sourcePath));

        var readStream = _fsExtra2['default'].createReadStream(sourcePath),
            writeStream = _fsExtra2['default'].createWriteStream(destPath);

        writeStream.on('finish', function () {
            resolve(_path2['default'].relative(__dirname, destPath));
        });
...
util.tmp.createDir = function createDir() {
    if (!_fsExtra2['default'].existsSync(util.tmp.path)) {
        _fsExtra2['default'].mkdirSync(util.tmp.path);
    }
};
...
util.tmp.cleanUp = function cleanUp() {
    _fsExtra2['default'].removeSync(util.tmp.path);
};
...
util.handleError = function handleError(err) {
    ...
    if (err.notify === true) {
        _notifications2['default'].err(err.msg);
    }
};
